...
Transforming data
Dataset transformations
Kloppy's .transform() method allows you to adapt the spatial representation of a dataset. This can be useful if you need to align data from different providers or to run analyses that assume a standard pitch size or attacking direction.
...
The to_orientation argument accepts both an Orientation (e.g., Orientation.HOME_AWAY) and an orientation mode's name (e.g., "home-away").
...
You can also define custom pitch dimensions. This is explained here.

When to use each pitch dimension
Choosing the right pitch dimension type depends on two main considerations:

Do you want to preserve real-world measurements (in meters/yards) or normalize to a unit scale?
Should the pitch be standardized to a common format, or should it reflect the original stadium-specific size?
...
How it works
The transformation scales all x and y coordinates proportionally such that the relevant position to the pitch markings are maintained. So, if a shot is half-way between the penalty spot and the penalty-area box, it stays that way after conversion. For a detailed explanation, see the mplsoccer docs.
...
The to_coordinate_system argument accepts a Provider (e.g., Provider.TRACAB), a provider's name (e.g., "tracab") or a coordinate system (e.g., TracabCoordinateSystem(pitch_length=108, pitch_width=69)).
